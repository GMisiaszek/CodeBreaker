...
namespace gui
{
    ...
    void GuiApi::fillAvailableHostsTable(std::vector<std::string> hosts)
    {
    ...
    void GridLayout::createAndFillButtonContainer()
    {
        ...
        addHostButton      = gtk_button_new_with_label("Dodaj hosta");
    ...
    {
        listStore         = GTK_LIST_STORE(gtk_list_store_new(1, G_TYPE_STRING));
        treeView          = GTK_TREE_VIEW(gtk_tree_view_new_with_model(GTK_TREE_MODEL(listStore)));
        renderer          = gtk_cell_renderer_text_new();
        hostnameColumn    = gtk_tree_view_column_new_with_attributes("HostName", renderer, "text", 0, NULL);
        gtk_tree_view_append_column(treeView, hostnameColumn);
        
    }

    void HostnameTableView::updateHostNameList(std::vector<std::string> hostNames)
    { 
